  ...
  <body>
    <div class="container">
      ...
      <form action="item.html" th:action th:object="${item}" method="post">
        ...
        <div>
          <label for="quantity" th:text="#{label.item.quantity}">수량
          </label>
...
* `<button type="submit">상품 등록</button>`
  ...
  * `<button type="submit" th:text="#{button.save}">취소</button>`
      ...
      <div class="row">
        <div class="col">
          <button class="w-100 btn btn-primary btn-lg"
                  ...
                  th:onclick="|location.href='@{/message/items/{itemId}/
edit(itemId=${item.id})}'|" 
                  type="button" th:text="#{page.updateItem}">상품 수정
          </button>
        </div>
        ...
        <table class="table">
          ...
          <tbody>
            <tr th:each="item : ${items}">
              <td><a href="item.html" 
                     th:href="@{/message/items/{itemId}					(itemId=${item.id})}" 
                     th:text="${item.id}">회원id</a></td>
              <td><a href="item.html" 
                     th:href="@{|/message/items/${item.id}|}" 
